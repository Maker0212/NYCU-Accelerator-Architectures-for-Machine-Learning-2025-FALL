...
inline void Softmax(const SoftmaxParams& params,
  ...
  // Softmax 內部累加使用的整數位數（沿用 TFLM 原本設計）
  static const int kAccumulationIntegerBits = 12;
  using FixedPointAccum =
      gemmlowp::FixedPoint<int32_t, kAccumulationIntegerBits>;
  using FixedPoint0 = gemmlowp::FixedPoint<int32_t, 0>;  // Q0.31
  ...
  for (int i = 0; i < outer_size; ++i) {
    // 1) 找該 row 的最大值（避免 overflow / 提升數值穩定）
    ...
    for (int c = 0; c < depth; ++c) {
      ...
      if (input_diff >= diff_min) {
        // 量化縮放到 Q5.26（TFLM 原本做法）
        const int32_t input_diff_rescaled =
            MultiplyByQuantizedMultiplierGreaterThanOne(
                input_diff, input_beta_multiplier, input_beta_left_shift);

        //    用 CFU 計算 exp_on_negative_values(scaled_diff)：
        //    in:  Q5.26 (<= 0)  →  out: Q0.31 之 e^{scaled_diff}
        const int32_t exp_q0 = static_cast<int32_t>(cfu_op0(
            /*function_id=*/2, static_cast<uint32_t>(input_diff_rescaled),
            0));  // CFU 直接提供 e^{scaled_diff} 的 Q0.31 結果

        // 轉成 FixedPoint0 再 Rescale 到累加格式
        sum_of_exps = sum_of_exps + gemmlowp::Rescale<kAccumulationIntegerBits>(
                                        FixedPoint0::FromRaw(exp_q0));
      }
    }

    // 3) 求 1/sum，得到縮放係數（Q0.31）
    int num_bits_over_unit = 0;
    FixedPoint0 shifted_scale = FixedPoint0::FromRaw(GetReciprocal(
        sum_of_exps.raw(), kAccumulationIntegerBits, &num_bits_over_unit));

    // 4) 輸出：softmax_i = exp_i / sum
    for (int c = 0; c < depth; ++c) {
      int32_t input_diff =
          static_cast<int32_t>(input_data[i * depth + c]) - max_in_row;
      if (input_diff >= diff_min) {
        ...
        // 再次用 CFU 取得 exp 值（Q0.31）
        const int32_t exp_q0 = static_cast<int32_t>(cfu_op0(
            /*function_id=*/2, static_cast<uint32_t>(input_diff_rescaled),
            0));  // 呼叫同一個 CFU 函式，避免重複的軟體 exp 計算

        // 直接用 FixedPoint0 做乘法與位移（維持原本做法）
        FixedPoint0 exp_in_0 = FixedPoint0::FromRaw(exp_q0);
